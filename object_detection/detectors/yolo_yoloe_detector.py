...
class YOLOYOLoEDetector:
    ...
    def run_inference(self, images_dir, img_size, models_dir, weights_file, prompt_type="text"):
        ...
        yolo_model_path = r"C:\Users\offic\OneDrive\Desktop\Evaluation-pipeline\object_detection\models\yolo11\Albatross-v0.4.pt"
        yolo_model = YOLO(yolo_model_path)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        yolo_model.to(device)


        yoloe_model_path = os.path.join(models_dir, self.model_name, weights_file)
        ...
        if prompt_type == "text":
            ...
            yoloe_model.set_vocab(vocab, names=names)
            # model.model.model[-1].is_fused = True
            # model.model.model[-1].conf = 0.001
            # model.model.model[-1].max_det = 1000
        ...
        for image_file in sorted(os.listdir(images_dir)):
            ...
            try:
                ...
                yoloe_results = yoloe_model.predict(image_path, conf=0.01, imgsz=img_size, verbose=False)  # pseudocode: adjust as needed
                yolo_results = yolo_model.predict(image_path, conf=0.05, imgsz=img_size, verbose=False)  # pseudocode: adjust as needed
                end_time = time.time()
                ...
                yoloe_results_boxes = yoloe_results[0].boxes  
                yolo_results_boxes = yolo_results[0].boxes  
                universal_results = self._convert_to_universal(yoloe_results_boxes, yolo_results_boxes, image_file)
    ...
    def _convert_to_universal(self, yoloe_results_boxes, yolo_results_boxes, image_file):
        ...
        for det in yoloe_results_boxes:  # Pseudocode: iterate over detections.
        ...
        for det in yolo_results_boxes:  # Pseudocode: iterate over detections.
            x1, y1, x2, y2 = map(int, det.xyxy.cpu().numpy().tolist()[0])
            universal["detections"].append({
                "bbox": [x1, y1, x2, y2],     # Example: [x1, y1, x2, y2]
                "score": det.conf.cpu().item(),    # Confidence
